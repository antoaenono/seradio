...
For **squash merge** PRs, your commits within a branch do not have to follow Conventional Commits. They may be unstructured ("set up boilerplate", "try greedy approach", "add test", etc.). The **PR title** becomes the squash commit message, so the title **must** follow [Conventional Commits format](https://www.conventionalcommits.org/).
